...
% Pad top and bottom
iresized_mask = [zeros(height_diff/2, size(small_nyc, 2) );...
    iresized_mask; zeros(height_diff/2, size(small_nyc, 2) )];
...
% The MATLAB  is a good place to discover what tools are
% available to you!
...
blue_channel(iresized_mask) = red(2);
love_small_nyc(:, :, 2) = blue_channel;

green_channel = love_small_nyc(:, :, 3);
green_channel(iresized_mask) = red(3);
love_small_nyc(:, :, 3) = green_channel;

imshow (love_small_nyc)
